...
class VM_PreparedOperand {
  ...
  toString() {
    ...
    return (
      "{ " +
      "Type: " +
      getDebugString(VM_OpType, this.Type) +
      ", Data: " +
      this.Data +
      ", Base: " +
      this.Base +
...
class VM_PreparedCommand {
  ...
  toString(indent) {
    ...
    return (
      ...
      indent +
      "  OpCode: " +
      getDebugString(VM_Commands, this.OpCode) +
      ",\n" +
      indent +
      "  ByteMode: " +
      this.ByteMode +
...
export default class RarVM {
  ...
  execute(prg) {
    ...
    const staticSize = Math.min(
      prg.StaticData.length,
      VM_GLOBALMEMSIZE - globalSize
    );
    if (staticSize) {
      this.mem_.set(
        prg.StaticData.subarray(0, staticSize),
        VM_GLOBALMEMADDR + globalSize
      );
    }
    ...
    let newBlockPos =
      dataView.getUint32(0x20, true /* little endian */) & VM_MEMMASK;
    const newBlockSize =
      dataView.getUint32(0x1c, true /* little endian */) & VM_MEMMASK;
    if (newBlockPos + newBlockSize >= VM_MEMSIZE) {
      newBlockPos = newBlockSize = 0;
    }
    prg.FilteredData = this.mem_.subarray(
      newBlockPos,
      newBlockPos + newBlockSize
    );

    prg.GlobalData = new Uint8Array(0);

    const dataSize = Math.min(
      dataView.getUint32(0x30),
    ...
    if (dataSize != 0) {
      ...
      prg.GlobalData.set(
        mem.subarray(VM_GLOBALMEMADDR, VM_GLOBALMEMADDR + len)
      );
    }
  }
        ...
        default:
          console.error(
            "RarVM OpCode not supported: " +
              getDebugString(VM_Commands, cmd.OpCode)
          );
          break;
      } // switch (cmd.OpCode)
      codeIndex++;
    ...
    switch (filterType) {
      ...
      case VM_StandardFilters.VMSF_AUDIO: {
        ...
        for (let curChannel = 0; curChannel < channels; ++curChannel) {
          ...
          for (
            ...
            let predicted = fromSigned32ToUnsigned32(
              8 * prevByte + k1 * d1 + k2 * d2 + k3 * d3
            ); // uint
      ...
      case VM_StandardFilters.VMSF_DELTA: {
        ...
        for (let curChannel = 0; curChannel < channels; ++curChannel) {
          let prevByte = 0;
          for (
            let destPos = dataSize + curChannel;
            destPos < border;
            destPos += channels
          ) {
      ...
      default:
        console.error(
          "RarVM Standard Filter not supported: " +
            getDebugString(VM_StandardFilters, filterType)
        );
        break;
    }
  }
      ...
      while (bstream.bytePtr < codeSize) {
        ...
        if (opNum > 0) {
          ...
          if (opNum == 2) {
            ...
            if (
              curCmd.Op1.Type == VM_OpType.VM_OPINT &&
              VM_CmdFlags[curCmd.OpCode] & (VMCF_JUMP | VMCF_PROC)
            ) {
